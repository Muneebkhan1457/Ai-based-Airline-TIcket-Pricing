...
def get_demand_at_price(route: str, flight_class: str, days_to_departure: int, price: float) -> dict:
    conn = sqlite3.connect(DB_PATH)
    try:
        base_fare = get_base_fare(conn, route, flight_class)
        if base_fare is None:
        ...
        context = {
            ...
            "competitor_min_price": comp_min,
            "competitor_avg_price": comp_avg,
            "price_vs_competitor_ratio": (price / comp_avg if comp_avg else None),
            "competitor_data_is_real": comp_avg is not None,
            "is_international": route == "KHI-DXB",
        }

        demand_ratio = predict_demand_at_price(context, price)
